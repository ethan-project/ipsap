...
func (ins *Application) getAppListQueryAndFilter(moreSelect string, moreJoin string, moreCondition string, appViewType string) (sql string, filter func(map[string]interface{})) {
	...
	filter = func(row map[string]interface{}) {
		...
		if nil != row["committee"] {
			...
			case "committee_ex_member":
				committee_judge_type_str = "일반심사"
			case "committee_in_member":
				committee_judge_type_str = "일반심사"
			case "expert_member":
				committee_judge_type_str = "전문심사"
			}
		...
		case DEF_APP_JUDGE_TYPE_CODE_IACUC:
			{
				...
				case DEF_APP_TYPE_NEW:
					sub_title = "동물실험 계획서"
				case DEF_APP_TYPE_CHANGE:
					sub_title = "동물실험 계획 변경 승인 신청서"
				case DEF_APP_TYPE_RENEW:
					sub_title = "동물실험 계획 재승인 신청서"
				case DEF_APP_TYPE_BRINGIN:
					sub_title = "실험동물 반입 보고서"
				case DEF_APP_TYPE_CHECKLIST:
					sub_title = "동물실험 계획 승인 후 점검표"
				case DEF_APP_TYPE_FINISH:
					sub_title = "동물실험 종료 보고서"
				}
			}
		case DEF_APP_JUDGE_TYPE_CODE_IBC:
			{
				switch common.ToUint(row["application_type"]) {
				case DEF_APP_TYPE_NEW:
					sub_title = "생물안전위원회 심의 신청서"
				case DEF_APP_TYPE_CHANGE:
					sub_title = "생물안전위원회 변경 심의 신청서"
				}
			}
		case DEF_APP_JUDGE_TYPE_CODE_IRB:
			{
				switch common.ToUint(row["application_type"]) {
				case DEF_APP_TYPE_NEW:
					sub_title = "기관생명윤리위원회 심의 신청서"
				case DEF_APP_TYPE_CHANGE:
					sub_title = "기관생명윤리위원회 변경 심의 신청서"
				case DEF_APP_TYPE_CONTINUE:
					sub_title = "기관생명윤리위원회 지속심의(중간보고) 신청서"
				case DEF_APP_TYPE_SERIOUS:
					sub_title = "기관생명윤리위원회 중대한 이상 반응 보고서"
				case DEF_APP_TYPE_VIOLATION:
					sub_title = "기관생명윤리위원회 연구계획 위반/이탈 보고서"
				case DEF_APP_TYPE_UNEXPECTED:
					sub_title = "기관생명윤리위원회 예상치 못한 문제발생 보고서"
				case DEF_APP_TYPE_FINISH:
					sub_title = "기관생명윤리위원회 종료 보고서"
				}
			}
...
func (ins *Application) getJsonData(item_name string, item Item, fileOnly bool, judge_type int) (ret map[string]interface{}) {
	...
	if exists {
		...
		if ms_exists {
			out_data["main_select"] = main_select
		}
