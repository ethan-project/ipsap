...
func (ins *AppBasic) InsertApplicationInfo(tx *sql.Tx, json interface{}) (ret bool, err_msg string) {
	...
	// DEF_APP_TYPE_CHANGE 변경 신청, DEF_APP_TYPE_RENEW 재승인, DEF_APP_TYPE_BRINGIN 반입신청서, DEF_APP_TYPE_CHECKLIST 승인후 점검표,
	// DEF_APP_TYPE_FINISH 종료 보고서, DEF_APP_TYPE_CONTINUE IRB 지속심의(중간보고), DEF_APP_TYPE_SERIOUS IRB 중대한 이상 반응 보고서,
	// DEF_APP_TYPE_VIOLATION IRB 연구계획 위반/이탈 보고, DEF_APP_TYPE_UNEXPECTE IRB 예상치 못한 문제발생 보고서
	case DEF_APP_TYPE_CHANGE, DEF_APP_TYPE_RENEW, DEF_APP_TYPE_BRINGIN, DEF_APP_TYPE_CHECKLIST, DEF_APP_TYPE_FINISH,
		...
		// 부모 신청서 application_type(서식유형)가 신규승인이 아니거나 과제 진행중이 아니면 RETURN
		if common.ToInt(row["application_type"]) != DEF_APP_TYPE_NEW || //  신규 승인
			!(common.ToInt(row["application_result"]) == DEF_APP_RESULT_EXPER_ING_A || // 과제 실험 중 (승인)
				common.ToInt(row["application_result"]) == DEF_APP_RESULT_EXPER_ING_AC) { // 과제 실험 중 (조건부 승인)
