...
function onClickApplication(e, a) {
  //alert("onClickApplication : " + e);
  switch (e) {
    case 0:
      switch (
        (g_AppInfo.initWithAppObj(supplement_saved_list[a]) ||
          alert("Fail to ApplicationInfo Init!"),
        supplement_saved_list[a].judge_type)
      ) {
        case IPSAP.JUDGE_TYPE.IACUC:
          //alert("onClickApplication IACUC");
          navigateIACUC(e, a);
          break;
        case IPSAP.JUDGE_TYPE.IBC:
          //alert("onClickApplication IBC");
          navigateIBC(e, a);
          break;
        ...
        default:
          return void alert(
            `JUDGE_TYPE Error!! (${supplement_saved_list[a].judge_type})`
          );
      }
      break;
    case 1:
      switch (
        (g_AppInfo.initWithAppObj(checking_saved_list[a]) ||
          alert("Fail to ApplicationInfo Init!"),
        checking_saved_list[a].judge_type)
      ) {
        case IPSAP.JUDGE_TYPE.IACUC:
          navigateIACUC(e, a);
          break;
        ...
        default:
          return void alert(
            `JUDGE_TYPE Error!! (${checking_saved_list[a].judge_type})`
          );
      }
      break;
    case 2:
      switch (
        (g_AppInfo.initWithAppObj(judge_ing_saved_list[a]) ||
          alert("Fail to ApplicationInfo Init!"),
        judge_ing_saved_list[a].judge_type)
      ) {
        case IPSAP.JUDGE_TYPE.IACUC:
          navigateIACUC(e, a);
          break;
        ...
        default:
          return void alert(
            `JUDGE_TYPE Error!! (${judge_ing_saved_list[a].judge_type})`
          );
      }
      break;
    case 3:
      if (!$(".final_apps a").first().hasClass("unavailable"))
        switch (
          (g_AppInfo.initWithAppObj(final_saved_list[a]) ||
            alert("Fail to ApplicationInfo Init!"),
          final_saved_list[a].judge_type)
        ) {
          case IPSAP.JUDGE_TYPE.IACUC:
            navigateIACUC(e, a);
            break;
          ...
          default:
            return void alert(
              `JUDGE_TYPE Error!! (${final_saved_list[a].judge_type})`
            );
        }
  }
}
  ...
  switch (e) {
    case 0:
      switch (supplement_saved_list[a].application_type) {
        ...
        default:
          return void alert(
            `application_type Error!! (${supplement_saved_list[a].application_type})`
          );
      }
      break;
    case 1:
      switch (checking_saved_list[a].application_type) {
        ...
        default:
          return void alert(
            `application_type Error!! (${checking_saved_list[a].application_type})`
          );
      }
      break;
    case 2:
      switch (judge_ing_saved_list[a].application_type) {
        ...
        default:
          return void alert(
            `application_type Error!! (${judge_ing_saved_list[a].application_type})`
          );
      }
      break;
    case 3:
      switch (final_saved_list[a].application_type) {
        ...
        default:
          return void alert(
            `application_type Error!! (${final_saved_list[a].application_type})`
          );
      }
  }
}
...
$(function () {
  ...
  API.load({
    ...
    success: function (e) {
      $(".supplement_cnt").text(e.supplement_cnt),
        $(".checking_cnt").text(e.checking_cnt),
        $(".judge_ing_cnt").text(e.judge_ing_cnt),
        $(".final_cnt").text(e.final_cnt);
      let a = "등록된 신청서",
      ...
      if ((t.empty(), e.supplement.length))
        $.each(e.supplement, function (a, i) {
          ...
          switch (i.application_type) {
            ...
            case IPSAP.APPLICATION_TYPE.RENEW:
              (s = "application"),
                (n = '<i class="mdi mdi-file-upload-outline"></i>'),
                "label gray_bg";
              break;
            case IPSAP.APPLICATION_TYPE.BRING:
            case IPSAP.APPLICATION_TYPE.END:
          ...
          c.addClass(`${i.judge_type_str}`).addClass(`${s}`).addClass(`${r}`),
            ...
            c
              .find(".link_btn")
              .addClass("btn btn-outline-danger btn_xxs")
              .attr("onclick", 'alert("신청자가 보완중인 문서입니다.")')
              .text("보완중"),
            t.append(c);
        });
      else {
        let e = t.data("comment") ? t.data("comment") : a;
        t.closest(".myApps").addClass("empty"),
          t.append(empty_apps).find(".empty_comment").text(e);
      }
      ...
      if ((i.empty(), e.checking.length))
        ...
        i.closest(".myApps").addClass("empty"),
          i.append(empty_apps).find(".empty_comment").text(e);
      }
      ...
      if ((s.empty(), e.judge_ing.length))
        $.each(e.judge_ing, function (a, t) {
          ...
          _.addClass(`${t.judge_type_str}`).addClass(`${i}`).addClass(`${c}`),
            ...
            _.find(".title_date").text(`(${t.tmp_submit_dttm})`),
            "0일 지연" == t.time_diff
              ? _.find(".bullet").text("당일 지연")
              : _.find(".bullet").text(`${t.time_diff}`),
            _.find(".link_btn").text("심사 설정 변경"),
            6 == t.application_result
              ? _.find(".link_btn").addClass("btn btn-danger btn_xxs")
              : _.find(".link_btn").addClass("btn btn-primary btn_xxs"),
            _.find(".link_btn").attr("onclick", `${r}`),
            s.append(_);
        });
        ...
        s.closest(".myApps").addClass("empty"),
          s.append(empty_apps).find(".empty_comment").text(e);
      }
      ...
      if ((n.empty(), e.final.length))
        $.each(e.final, function (a, t) {
          ...
          switch (t.application_type) {
            ...
            case IPSAP.APPLICATION_TYPE.RENEW:
              (i = "application"),
                (s = '<i class="mdi mdi-file-upload-outline"></i>'),
                "label gray_bg";
              break;
            case IPSAP.APPLICATION_TYPE.BRING:
            case IPSAP.APPLICATION_TYPE.END:
        ...
        n.closest(".myApps").addClass("empty"),
          n.append(empty_apps).find(".empty_comment").text(e);
      }
      JSON.parse(COMM.getCookie("user_info")).user_type[1] ||
        $(".final_apps a")
          ...
          .on(
            ...
            (e) => (
              alert(
                "최종 심의는 행정 간사 권한을 가진 사용자만 수행할 수 있습니다."
              ),
              e.preventDefault(),
          ...
          ? $.each(e.performance_app_list, function (e, a) {
              ...
              t.addClass(a.judge_type_str),
                ...
                t
                  .find(".title_date")
                  .text(
                    `(${getDttm(a.approved_dttm).dt} ~ ${a.general_end_date})`
                  ),
                t
                  .find(".end_date")
                  .html(
                    `${a.general_end_date}<i class="mdi mdi-arrow-collapse-right mLeft5"></i>`
                  );
                ...
                "closed" == i &&
                  t
                    ...
                    .attr({
                      ...
                      title:
                        '실험 기간이 만료되었습니다. "종료 보고서" 제출을 확인해 주세요.',
                    });
    ...
    $("#btn_download").click(function (e) {
      ...
      location.href =
        CONST.API_PATH +
        CONST.API.DASHBOARD.DOWNLOAD +
        "?filter.institution_seq=" +
        a.institution_seq;
    });
});


function navigateIRB(e, a) {
    switch (e) {
      case 0:
        switch (supplement_saved_list[a].application_type) {
          case IPSAP.APPLICATION_TYPE.NEW:
  ...
  function navigateIRB_new(e, a) {
    switch (e) {
      ...
      case 1:
        ...
        break;
      case 2:
        g_AppInfo.saveParamsAndNavigate("./IRB/reviewOffice_list-IRB_setup.html");
        break;
      case 3:
        g_AppInfo.saveParamsAndNavigate(
  ...
  function navigateIRB_change(e, a) {
    switch (e) {
      case 0:
        g_AppInfo.saveParamsAndNavigate("./IRB/application_list-IRB_info.html");
        break;
      case 1:
        switch (parseInt(checking_saved_list[a].application_result)) {
          case IPSAP.APPLICATION_RESULT.CHECKING:
            g_AppInfo.saveParamsAndNavigate(
              "./IRB/reviewOffice_list-IRB_review_change.html"
            );
            break;
          case IPSAP.APPLICATION_RESULT.CHECKING_2:
            g_AppInfo.saveParamsAndNavigate(
